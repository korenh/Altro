...
export default class ChatInfo extends Component {
  ...
  componentDidMount() {
    firebase.firestore().collection("users").doc(this.props.id).get().then((doc) => {
  ...
  render() {
    return (
      <div className="chat-main2" onClick={() => this.props.jobInfo()}>
        ...
        {this.props.allUsers.map((v) => (<div key={v.uid} className="owner-info-chat"><img alt="img" src={v.profileImageURL} /><p>{v.name}</p><p className="owner-info-blue">Employee</p></div>))}
      </div>
    );
  }
}
